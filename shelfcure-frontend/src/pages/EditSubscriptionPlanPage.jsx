...
const EditSubscriptionPlanPage = () => {
  ...
  // Form state - matching Create Plan page exactly
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          // Pricing & Billing - convert from backend structure
          basePrice: plan.pricing?.monthly || 0,
          defaultBillingCycle: '1', // Default to monthly
          ...
          discounts: {
            ...
            3: 5,
            6: 10,
            12: plan.pricing?.discountPercentage || 20
          },
          ...
          // Limits & Quotas
          maxStores: plan.limits?.maxStores?.toString() || '1',
          usersPerStore: plan.limits?.maxUsers || 5,
          medicinesPerStore: plan.limits?.maxProducts?.toString() || '100',
          customersPerStore: '1000', // Default value
          suppliersPerStore: '10', // Default value
          sortOrder: plan.sortOrder || 0,

          // Trial Settings
          enableTrial: plan.trial?.enabled || false,
          trialDays: plan.trial?.durationDays || 14,

          // Core Features
          coreFeatures: {
            inventoryManagement: true, // Always true for pharmacy
            salesAnalytics: plan.features?.analytics || false,
            customerManagement: true, // Always true for pharmacy
            supplierManagement: plan.planType !== 'basic'
          },

          // Advanced Features
          advancedFeatures: {
            expenseTracking: plan.features?.advancedAnalytics || false,
            staffManagement: plan.limits?.maxUsers > 1,
            prescriptionManagement: plan.planType !== 'basic',
            wasteManagement: plan.features?.advancedAnalytics || false,
            storageManagement: plan.features?.advancedAnalytics || false,
            advancedReports: plan.features?.customReports || false,
            multiLocationSupport: plan.features?.multiStore || false,
            apiAccess: plan.features?.apiAccess || false
          },

          // Plan Visibility & Promotion
          isPopular: plan.isPopular || false,
          isRecommended: false, // Not in backend model
          promotionalText: '', // Not in backend model
  ...
  // Handle input changes - matching Create Plan page
  ...
  // Validate form - matching Create Plan page
  const validateForm = () => {
    const newErrors = {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Transform frontend data back to backend structure
      const backendData = {
      ...
      if (data.success) {
        ...
        navigate(`/admin/subscription-plans/${id}`);
      } else {
  ...
  return (
    <AdminLayout title="Edit Subscription Plan" subtitle="Update subscription plan with pricing and features">
      <div className="space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between">
          <button
            onClick={() => navigate(`/admin/subscription-plans/${id}`)}
